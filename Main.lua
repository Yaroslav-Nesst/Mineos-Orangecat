...
local localizations = {
    ...
    ["Russian"] = {
        ...
        users = "Юзеры",
        usersNotSpecified = "Если юзеры не указаны, любой может получить доступ к загрузочному сектору",
        ...
        options = "Настройки",
        flashFailed = "Прошив неудачен, причина:",
        downloadFailed = "Файл не скачался, причины нет:"
    }
}
...
local workspace, window = system.addWindow(GUI.titledWindow(1, 1, 52, 17, "Top Recovery Installer", true))
...
container:addChild(GUI.roundedButton(18, 15, 15, 1, 0xE1E1E1, 0x696969, 0x878787, 0xFFFFFF, localization.flash)).onTouch = function()
    ...
    if #config > 64 then
        ...
        workspace:draw()
        local data, reason = internet.request("https://pastebin.com/raw/e98eL0DX")
        downloading:remove()
